      ...
      <!--breadcrumbs end -->
		  	<section class="panel">
		          <header class="panel-heading">
                    ...
                    <div class="col-md-3">
                      User
                      <select class="form-control filterz" name="user_id">
                      <option selected="selected" value="">All users</option>
                        <?php foreach ($users as $value): ?>
                          <option <?php echo ($value->id == $this->input->get('user_id')) ? 'selected="selected"': '' ?> value="<?php echo $value->id ?>"><?php echo $value->name ?></option>
                        <?php endforeach ?>
                      </select>
                    </div>
                    <div class="col-md-3">
                      <br>&nbsp;
                        <a class="btn btn-warning btn-sm" href="<?php echo base_url('cms/link-builder-tool-admin')?>">Clear Filters</a>
                    </div>
		          	</div>
		          </header>
		      </section>
    ...
    <p class="highcharts-description"> 
              ...
              <ul class="pagination">
                <ul class='pagination'>
                  ...
                  <li><a href="<?php echo base_url('cms/link-builder-tool-admin/') . "?page=1&" . @$filters;?>">&laquo;</a></li>
                  ...
                  <?php for ($i = $page - 2; $i < ($page) ; $i++):
                   ...
                   ?>
                  <li><a href="<?php echo base_url('cms/link-builder-tool-admin/') . "?page=" . $i . "&" . @$filters;?>"><?= $i ?></a></li>
                  <?php endfor; ?>
                  <!-- / loop for desc -->

                  <li><a href="<?php echo base_url('cms/link-builder-tool-admin/') . "?page=" . $page . "&" . @$filters;?>"><?= $page ?></a></li>
                ...
                <li><a href="<?php echo base_url('cms/link-builder-tool-admin/') . "?page=" . $total_pages . "&" . @$filters;?>">&raquo;</a></li>
                </ul>
              </ul>
              <!-- pagination -->
                  </div>
...
<script>
  $(document).ready(function($) {
    ...
    $('.filterz').on('change', function() {
      ...
      + "&user_id=" + ($('select[name=user_id]').val() ? $('select[name=user_id]').val() : '')
    })
  });
</script>
